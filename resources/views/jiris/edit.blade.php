...
<main class="px-6 py-6 flex items-center flex-col">
    @if ($jiri)
        <h1 class="font-bold text-4xl pb-20">Modifier le Jiri : {{$jiri->name}}</h1>

        <form action='{{route('jiris.update', $jiri->id)}}' method="post" class="bg-blue-50 rounded-2xl shadow-2xl p-10 min-w-2xl">
            @method('PATCH')
            @csrf
